...
async function load() {
  ...
  try {
    ...
    if (!r.ok) throw new Error(body.error || '现金流量表加载失败')
    data.value = body
  } catch (e) {
    error.value = e.message || '现金流量表加载失败'
  } finally {
    loading.value = false
  }
}
...
function exportExcel() {
  const rows = [
    ['现金流量表', data.value.period || `${startDate.value}~${endDate.value}`, '', ''],
    ['项目', '行次', '金额', '公式'],
  ...
  link.href = url
  link.download = `现金流量表-${startDate.value}-${endDate.value}.xls`
...
<template>
  ...
  <div class="card">
    ...
    <table>
      <thead><tr><th>项目</th><th class="num">行次</th><th class="num">金额</th><th>公式</th></tr></thead>
      <tbody>
